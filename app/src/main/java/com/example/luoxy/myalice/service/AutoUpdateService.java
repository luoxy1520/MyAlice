package com.example.luoxy.myalice.service;
...
import com.example.luoxy.myalice.WeatherActivity;
...
public class AutoUpdateService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int starId) {
        ...
        AlarmManager manager = (AlarmManager) getSystemService(ALARM_SERVICE); // 取得系统的定时服务 manager 用来设置定时服务
        ...
        manager.cancel(pi);
        manager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, triggerTime, pi); // 用 setExact() 精确定时
        ...
        if (weatherString != null) {
            ...
            HttpUtil.sendOkHttpRequest(weatherUrl, new Callback() {
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    String responseText = response.body().string(); // 要传入的 response 是 String 类型的哦
                    Weather weather = Utility.handleWeatherResponse(responseText); // GSON解析出来 返回一个类
